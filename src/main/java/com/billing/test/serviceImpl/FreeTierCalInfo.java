...
//	Global-DataTransfer-Out-Bytes
		...
		public static BigDecimal FreeTierApply(List<AwsComDefaultVO> pvo, String usagetype, BigDecimal beginRange, BigDecimal endRange, BigDecimal intervalAmount,Map<String,BigDecimal> getFreeTierList) {
...
//			BigDecimal freeBeginRange			= new BigDecimal("0");	//프리티어 사용최소범위는 0으로 전제함
			...
			for(String freeTier : freeTierList.keySet()) {					//프리티어 값을 str에 담아 반복문을 돌린다.
				...
				if(cmp2.equals(cmp1)) {									//유형타입이 맞을 경우
					...
					for(AwsComDefaultVO pvoList : pvo) {			//PriceList에서 프리티어의 구간정보를 찾아내기 위한 반복문 실행
