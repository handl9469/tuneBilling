...
public class CalListPrintImpl {
	...
	public static void calList(List<AwsComDefaultVO> pvo, List<ResultByTime> evo) {
		...
		//계산방식:유형별로 구간별 사용량*구간단위가격 의 총합을 소수점둘째자리까지 반올림하여 청구서 금액과 비교
	 	...
	 	for(ResultByTime resultByTime : evo) {		//월별	 		
 			...
 			for(Group group : resultByTime.getGroups()) {		//월별 사용유형값
 				...
 				for(AwsComDefaultVO pvoList : pvo) {
	...
	//프리티어 리스트를 담는 클래스
	public static Map<String,BigDecimal> getFreeTierList(List<AwsComDefaultVO> pvo) {
//			List<String> freeTierList = new ArrayList<String>();	//프리티어 담을 리스트 생성
		...
		for(AwsComDefaultVO pvoList : pvo) {					//PriceList 검색
			String pvoUsageType = pvoList.getUsagetype();
